...
function App() {
  ...
  const recommendationLevels = level ? [Number(level) - 1, Number(level), Number(level) + 1].filter((target) => target > 0) : [];
  const recommendations = useMemo(() => recommendationLevels.map((target) => ({
    target,
    xp: payload?.hunts.filter((hunt) => hunt.recLevel === target).sort((a, b) => b.efficiency.xpPerHp - a.efficiency.xpPerHp).slice(0, 1) || [],
    gold: payload?.hunts.filter((hunt) => hunt.recLevel === target).sort((a, b) => b.efficiency.totalGoldPerHp - a.efficiency.totalGoldPerHp).slice(0, 1) || [],
  })), [level, payload]);
  ...
  return <div className="wiki-app">
    ...
    <main className="content">
      ...
      {activeSection === 'hunts' ? <>
        ...
        {level && <RecommendationSection recommendations={recommendations} onOpen={setSelectedHunt} />}
...
function RecommendationSection({ recommendations, onOpen }) {
  return <section className="recommendations"><div className="recommendation-heading"><div><div className="section-kicker"><span /> RECOMENDAÇÕES POR LEVEL</div><h2>Melhores lugares para caçar</h2><p>As três melhores opções de cada faixa, separadas por XP e Gold.</p></div><span className="recommendation-note">RECÁLCULO AUTOMÁTICO</span></div><div className="recommendation-columns"><RecommendationGroup title="XP / HP" accent="xp" recommendations={recommendations} metric={(hunt) => percent(hunt.efficiency.xpPerHp)} onOpen={onOpen} /><RecommendationGroup title="GOLD / HP" accent="gold" recommendations={recommendations} metric={(hunt) => fmt(hunt.efficiency.totalGoldPerHp, 3)} onOpen={onOpen} /></div></section>;
}
function RecommendationGroup({ title, accent, recommendations, metric, onOpen }) {
  return <div className={`recommendation-group ${accent}`}><div className="group-title"><span>{accent === 'xp' ? '✦' : '◈'}</span><strong>{title}</strong><small>MELHOR EFICIÊNCIA</small></div><div className="recommendation-levels">{recommendations.map(({ target, [accent]: hunts }) => <div className="recommendation-level" key={`${accent}-${target}`}><div className="level-heading"><span>LEVEL</span><strong>{target}</strong>{target === recommendations[1]?.target && <em>EXATO</em>}</div>{hunts.length ? hunts.map((hunt, index) => <button className="recommendation-row" key={`${hunt.stage}-${hunt.name}`} onClick={() => onOpen(hunt)}><span className="rank">0{index + 1}</span><span className="recommendation-name">{hunt.name}<small>ESTÁGIO {hunt.stage} · REC. {hunt.recLevel}</small></span><b>{metric(hunt)}</b></button>) : <div className="no-recommendation">Sem hunts nesta faixa.</div>}</div>)}</div></div>;
}
